...
# Вариант b) игра с ботом, который наделен "интеллектом"
...
def get_status(name, k, counter, value):
    print(
        f"Ход сделал {name}, он взял {k} конфет, теперь у него {counter}. Осталось всего {value} .")


def bot_intelligence(value):
    decision = randint(1, 29)
    while value-decision <= 28 and value > 29:
        decision = randint(1, 29)
    return decision
